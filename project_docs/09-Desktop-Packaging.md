...
- **打包态无头自检**（CI/本地冒烟）：
  ...
  # 期望输出：[selftest] health=ok data_loaded=True stores=5 products=20 gui_backend=ok
  ```
  `gui_backend=ok` 表示 pywebview 的平台后端（macOS=cocoa / Windows=edgechromium）
  已正确打入并可加载——这是"能弹出原生窗口"的前提。
...
因 PyInstaller 不能交叉编译，需在各平台 runner 各自本地构建。工作流拆成两条：

| 工作流 | runner | 产物 | 覆盖机型 |
|---|---|---|---|
| `build-desktop.yml`（主） | `windows-latest` | `库存预测系统-windows-x64.zip` | Windows x64 |
| `build-desktop.yml`（主） | `macos-14` | `库存预测系统-macos-arm64.zip` | Apple Silicon (M 系列) |
| `build-desktop-intel.yml` | `macos-13` | `库存预测系统-macos-x64.zip` | Intel Mac |

> **为何拆分**：GitHub 托管的 Intel（`macos-13`）runner 排队时间极长、时常长时间领不到机器。若放在主 matrix 里会拖累 Windows/Apple Silicon 出包与发布。故 Intel 单列一条流水线，用**相同的 `v*` 标签**触发，构建完把 zip **追加**到同名 Release（`softprops/action-gh-release` 对已存在 Release 只新增附件、不覆盖其余附件与说明）。即使 Intel 一直排不到机器，主流水线也已先带另两个包发布。

**触发方式**
- 推送 `v*` 标签（如 `v1.0.0`）→ 两条工作流并行触发：主流水线出 Windows + Apple Silicon 并建 Release；Intel 流水线完成后追加其 zip；
...
# 两条工作流被同一标签触发：
#   - 主流水线构建 Windows + Apple Silicon，创建 Release v1.0.0 并附 2 个 zip；
#   - Intel 流水线（排到 runner 后）构建 macos-x64 并把第 3 个 zip 追加到该 Release。
```
...
- **pywebview 后端收集**：pywebview 在运行时才 `import webview.platforms.<backend>`（见 `webview/guilib.py`），
  PyInstaller 静态分析跟不到，故 `desktop.spec` 用 `collect_submodules("webview")` 收全，并显式补 pyobjc
  隐藏导入（cocoa 后端依赖 `objc/Foundation/AppKit/WebKit/Quartz`）。cocoa 后端 import 时还会链式加载
  `webview.http`（依赖 `bottle`），故 `bottle` 不可加入 `excludes`。若升级 pywebview 后窗口打不开、
  自检显示 `gui_backend=FAIL ...`，据缺失模块名补 `hiddenimports` 或从 `excludes` 移除对应依赖。
